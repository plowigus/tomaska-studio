...
export function OfferSection() {
    ...
    return (
        <section ref={containerRef} id="oferta" className="px-8 md:px-16 lg:px-24 py-16 md:py-24 bg-alabaster overflow-hidden">
            <div ref={headerRef} className="mb-16 md:mb-24 border-b-2 border-black pb-8">
                <h2 className="text-[clamp(2rem,6vw,5rem)] tracking-tight font-bold uppercase font-serif text-black leading-none">Współpraca</h2>
            </div>
            ...
            <div ref={cardsRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 items-stretch">
                {OFFER_STEPS.map((step) => (
                    <div key={step.id} className="group relative h-[420px] flex flex-col p-8 bg-transparent transition-colors duration-300 cursor-pointer">
                        ...
                        <div className="relative z-20 h-full flex flex-col text-black">
                            ...
                            <h3 className="text-2xl font-serif leading-[1.1] font-bold mb-4">{step.title}</h3>
